...
这是一个面向 **AI 产品经理求职展示** 的个人 AI 产品项目。项目以扫地机器人 / 扫拖一体机售后为场景，模拟用户在故障排查、维护保养、选购建议、环境适配和月度使用报告中的咨询流程。

它不是简单的聊天壳，而是一个能展示完整 AI 应用链路的 Demo：
...
- 线上 Demo 是求职展示用，不承诺生产级并发、监控、审计和 SLA。
- 评测脚本可用于跑真实结果，但仓库目前不提交 `eval_results/`，简历数字需要以实际运行结果为准。
- 本项目重点展示 AI 产品经理需要理解的 Agent、RAG、工具调用、Prompt、评测和 Demo 产品化能力。
...
评测方式是关键词覆盖率，适合用于比较 Prompt、分块参数、Top-K、知识库内容调整前后的变化。它不能完全代表真实用户满意度，但可以作为简历项目的量化证据。
...
1. **不是纯聊天壳。** 项目包含 Agent 决策、RAG 检索、工具调用、动态 Prompt、用户档案、使用记录和评测脚本。
2. **把产品场景拆成工具。** 售后问答走 RAG，天气问题走 Open-Meteo，报告问题走用户 ID、月份和 CSV 使用记录。
3. **把可测试性做进结构。** `@tool` 函数只做声明和委托，天气、CSV、来源格式化、配置解析等逻辑都能独立单测。
4. **能讲清楚边界。** 使用记录是模拟外部系统数据，线上 Demo 是求职展示，不伪装成真实商业上线。
5. **有评测意识。** 用 30 题评测集和 compare 脚本验证改动是否真的提升关键词覆盖率。
